...
// Atributos
  ...
  public function showFormAddPelicula() {
    ...
    <html>
        ...
        <body>
            ...
            <hr> 
            <form method="get" name="formGeneral" action="index.php">
                ...
                <input type="text" name="do" value="procesarFormAnadirPelicula"/>
                <br>
  ...
  /**
   * ♥ Objeto : crea la conexion a la base de datos
  ...
  public function borra_pelicula() {
    $db = new mysqli("localhost", "root", "", "videoclubprueba");
    if ($db->connect_error) {
      print "Error en la conexion : " . $db->connect_errno;
      die(" Error : No se establecio la conexion . " . $db->connect_error);
    }
    if (isset($_GET['enviar']) && !empty($_GET['cod_pelicula'])) {
      # Es correcto este bucle condicional ? 
      if ($this->consultar_borrar_return() == $_GET['cod_pelicula']) {
